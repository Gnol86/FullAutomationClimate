...
class ClimateConstants:
    ...
    Attributes:
        ...
        DEFAULT_MODES: Default operating modes for climate units
    """
    ...
    DEFAULT_MODES = {
        'preset': 'manual',  # Default preset mode
        'hvac': 'heat'      # Default HVAC mode
    }
...
class FullAutomationClimate(hass.Hass):
    ...
    def _init_global_config(self) -> None:
        ...
        self.global_modes = {
            'preset': self.args.get('preset_mode', ClimateConstants.DEFAULT_MODES['preset']),
            'hvac': self.args.get('hvac_mode', ClimateConstants.DEFAULT_MODES['hvac'])
        }
    ...
    def _get_mode(self, climate: Dict[str, Any], mode_type: str) -> str:
        """
        Determines operating mode according to configuration hierarchy.
        
        Args:
            climate: Climate unit configuration dictionary
            mode_type: Type of mode to determine ('preset' or 'hvac')
            
        Returns:
            str: Operating mode to apply
            
        This method implements the following hierarchy:
        1. Local mode if defined
        2. Global mode if defined
        3. Default mode as fallback
        """
        mode_key = f'{mode_type}_mode'
        
        # Check local mode
        if mode_key in climate:
            return climate[mode_key]
        
        # Check global mode
        if self.global_modes[mode_type]:
            return self.global_modes[mode_type]
        
        # Default mode
        return ClimateConstants.DEFAULT_MODES[mode_type]
    ...
    def set_climate(self, climate_index: int) -> None:
        ...
        2. Determines appropriate operating mode and temperature
        ...
        try:
            ...
            # Determine mode and temperature
            ...
            # Configure modes
            preset_mode = self._get_mode(climate, 'preset')
            hvac_mode = self._get_mode(climate, 'hvac')

            # Apply modes if needed
            current_preset = self.get_state(climate_entity, attribute='preset_mode')
            current_hvac = self.get_state(climate_entity, attribute='hvac_mode')

            if current_preset != preset_mode:
                self.call_service("climate/set_preset_mode", entity_id=climate_entity, preset_mode=preset_mode)
                self.debug_log(f"  Set preset mode to {preset_mode}")

            if current_hvac != hvac_mode:
                self.call_service("climate/set_hvac_mode", entity_id=climate_entity, hvac_mode=hvac_mode)
                self.debug_log(f"  Set hvac mode to {hvac_mode}")
